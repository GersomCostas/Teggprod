 ...
 
{
  ...
  out<-join((select(tdate,Year, period, Component, dates, datee, days_per) %>%   dplyr::rename(days = days_per)),out)
    
  out<-rbind.fill(out,(select(tdate,Year, inter_period, Component, dates, datee, days_interper) %>%   dplyr::rename(period = inter_period, days = days_interper)))
...
tableDEP2<-
  function(data)
  {
    ...
    out<-rbind.fill(out,(select(tdate,Year, inter_period, Component, dates, datee, days_interper) %>%   dplyr::rename(period = inter_period, days = days_interper)%>%mutate(dates=NA,datee=NA)))
    ...
    for (i in 3:length(out$DEP))
    {
      out$mid_day[i]<-yday(out$dates[1])+sum(out$days[2:(i-1)],na.rm=T)+out$days[i]/2
      out$mid_day[2]<-yday(out$dates[1])+out$days[2]/2
    }
    ...
    write.csv(out, "output/output_table_midday.csv", na="", row.names = F)
  }
